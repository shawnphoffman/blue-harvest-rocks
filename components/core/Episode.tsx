...
export default function Episodes({ episode }) {
	...
	return (
		<section className="flex flex-col justify-start w-full py-4 text-sm text-left" id={episode.guid}>
			<h2 className="mb-2 text-2xl font-bold text-center text-brand-bh4 md:mb-4">{episode.title}</h2>
			<div className="flex flex-col items-center justify-start gap-4 md:flex-row md:items-start">
				...
				<div className="flex flex-col self-stretch overflow-hidden whitespace-break-spaces text-wrap text-ellipsis">
					<div className="mb-2 text-md font-semibold text-white/75">{pubDate}</div>
					<div className="[&_a]:text-brand-bh4 mb-1 [&_a]:pb-0.5 [&_a]:font-bold [&_a:hover]:text-brand-bh3 [&_a:hover]:underline [&_a]:break-words">
						<EpisodeSummary summary={episode.summary} />
					</div>
					<div className="flex items-end flex-1">
						<a
							className="inline-block text-base font-bold text-brand-bh3 hover:text-brand-bh4 hover:underline hover:underline-offset-2"
							target="_blank"
